...
def test_4c_rule_8e_discharges_the_obligation_when_the_turn_does_not_fit():
    """With no starboard room, an effective crossing slowdown earns credit."""
    common = dict(dcpa=0.2 * D_REQ, a_req=0.8, dy_req=0.8 * D_REQ,
                  psi_engage=0.0, u_engage=cfg.U_REF, tcpa=5.0,
                  a_stbd=False, r_stbd=-0.2)
    ctx = ctx_for(enc.CROSSING, rng=4.0, alpha=45.0, ct=270.0,
                  speed_ts=cfg.U_REF, u_own=0.0, **common)
    assert ctx.slowdown_clears
...
def test_a_cleared_encounter_returns_to_idle():
    ...
    # Now astern and opening outside required separation.  Opening while
    # still within the domain must retain the original obligation.
