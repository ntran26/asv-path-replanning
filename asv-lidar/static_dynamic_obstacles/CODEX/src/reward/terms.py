...
class RewardState:
    ...
    # --- perceived motion: compare with perceived encounter latches -------
    # None retains the physical value for legacy isolated reward callers.
    ...
    * **`R-5`** -- narrow-channel overtaking where the port pass does not fit.
    ...
    for ctx in _iter(contexts):
        ...
        if (ctx.engaged and ctx.gives_way and not ctx.turn_admissible
                and getattr(ctx, "slowdown_clears", True)):
...
def v_r8(state: RewardState, ctx, cfg) -> float:
    ...
    A_t     = credited_turn + (Du_red/Du_min if slowdown_clears else 0)
    ...
    * Speed reduction counts only when the policy's coast trajectory predicts
      safe target clearance.  Slowing head-on along a collision course delays
      contact but does not discharge the action obligation.
...
def r8_parts(state: RewardState, ctx, cfg) -> dict:
    ...
    parts = {"a_req": float(ctx.a_req), "dpsi_c": 0.0, "du_red": 0.0,
             "slowdown_clears": False, "slowdown_credit": 0.0,
    ...
    parts["slowdown_clears"] = bool(ctx.slowdown_clears)
    if parts["slowdown_clears"]:
        parts["slowdown_credit"] = parts["du_red"] / max(float(cfg.du_min), 1e-9)
    a_t = parts["slowdown_credit"]
    ...
    if ctx.turn_admissible or not parts["slowdown_clears"]:
...
def explain_colregs(state: RewardState, ctx, cfg) -> Dict[str, str]:
    ...
    if cls not in GIVE_WAY:
        ...
        elif not parts["slowdown_clears"] and not ctx.turn_admissible:
            out["r8"] = (f"slowdown cannot clear; turn credited, "
                         f"A_t {parts['a_t']:.2f}")
