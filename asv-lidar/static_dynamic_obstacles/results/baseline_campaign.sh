...
# (verified identical to baseline-v1).  Order: the cheap learners first (PPO,
# RecurrentPPO: ~2.5 days), then seed 0 of TD3/SAC/TQC (a first look at every
# learner, ~5 days), then the remaining seeds seed by seed, so the campaign can
# move to a cluster at any point with every learner partly done.
cd "$(dirname "$0")/.."
...
JOBS="ppo:2 ppo:3 ppo:4 recurrent_ppo:0 recurrent_ppo:1 recurrent_ppo:2 recurrent_ppo:3 recurrent_ppo:4
      td3:0 sac:0 tqc:0 td3:1 sac:1 tqc:1 td3:2 sac:2 tqc:2 td3:3 sac:3 tqc:3 td3:4 sac:4 tqc:4"
for job in $JOBS; do
  A=${job%%:*}; S=${job##*:}
